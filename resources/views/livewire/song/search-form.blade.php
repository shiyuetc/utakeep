<div>
    <form wire:submit="search" class="flex gap-2 mb-4">
        <input
            ...
            class="flex-1 h-10 px-3 text-sm border border-gray-200 bg-white text-gray-900 rounded-md outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 transition"
            required
        >
        <button type="submit" class="h-10 px-5 bg-primary text-primary-light text-sm font-medium hover:bg-primary-hover rounded-md transition cursor-pointer flex-shrink-0">
    ...
    @if ($searched)
        @if (count($songs) > 0)
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="flex items-center gap-3 bg-white border border-gray-200 rounded-md px-4 py-3">
                    <div class="w-11 h-11 border border-gray-200 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
                        @if ($song['image_url'])
